...
class LinearModel(object):
  ...
  def __init__(self,
    ...
    self.HUMAN_2D_SIZE_eccv18 = 17 * 2
    ...
    if(for_eccv18 == True):
      if (centering_2d == False):
        self.input_size = self.HUMAN_2D_SIZE_eccv18 # for challenge
      else:
        self.input_size = self.HUMAN_2D_SIZE  # for challenge

    else:
      self.input_size = self.HUMAN_2D_SIZE  # for benchmark
    ...
    with vs.variable_scope( "linear_model" ):
      ...
      if (for_eccv18 == True):
        if (centering_2d == False):
          w1 = tf.get_variable(name="w1", initializer=kaiming, shape=[self.HUMAN_2D_SIZE_eccv18, linear_size], dtype=dtype)
        else:
          w1 = tf.get_variable(name="w1", initializer=kaiming, shape=[self.HUMAN_2D_SIZE, linear_size],
                               dtype=dtype)
      else:
  ...
  def step_only_enc(self, session, encoder_inputs, dropout_keep_prob, isTraining=True):
    """Run a step of the model feeding the given inputs.
    ...
    return output  # No gradient norm
  ...
  def get_all_batches_eccv18( self, data_x, data_y, training=True):
    """
    Obtain a list of all the batches, randomly permutted
    Args
      data_x: Poses with 2d inputs
      data_y: Poses with 3d expected outputs
      training: True if this is a training batch. False otherwise.
  ...
  def get_all_batches_2D_eccv18( self, data_x):
    """
    Obtain a list of all the batches, randomly permutted
    Args
      data_x: Poses with 2d inputs
